...
字段：内置 `skin`、`scrim`、`wallpaper`（含 `focus` / `clip` / `sha256`）、可选 `textColor`（`#rgb` / `#rrggbb`，缺省或非法 = 跟随浅深色主题；**只**染侧栏 / 顶栏图标 / 首页欢迎标语，经 `--appearance-chrome-ink`，**不**改全局 `--text-primary`）、可选 `fontShadow`（bool，缺省 `false`）。`tokens` / `style` / `css` 或 `schemaVersion !== 1` → `unsupported_schema`。未知 `skin` **不**整包失败：回落 `default` + `warnings: ["unknown_skin"]`。`wallpaper == null` 默认清除接收方壁纸（`will_clear_wallpaper`）。设置页本身始终实色，不进皮肤包。
