...
- Wave 3: an `sshAlias` project spawns `ssh -T` + ControlMaster, then remote `grok agent stdio` in the remote cwd. Do not spawn local grok with that path. Imported chats store the remote grok UUID as `agent_session_id` so ACP `session/load` can resume. Send and warm-connect are on. Persistence is still Wave 4 (`--no-leader` on SSH). Tests: `ssh_acp_prefix_keeps_alias_and_cwd_as_argv`.
