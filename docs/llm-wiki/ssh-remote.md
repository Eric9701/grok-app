...
- Sidebar remote rail: one group per watching host, heading `远程 {alias}` / `Remote {alias}`. Session title is custom name (local overlay, double-click to edit) else first user sentence from remote `chat_history.jsonl` / `summary.json` else cwd basename. Never show the raw agent session id as the title. Full remote cwd is a hover Tip, not a wrapping subtitle. List newest 20, then **Load more**. Click adds the SSH project and opens a new local chat (wave 3 still does not resume grok on the host). Remote right-click menu parity with local sessions is a later TODO.
