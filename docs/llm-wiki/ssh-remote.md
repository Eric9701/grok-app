...
- Sidebar remote rail uses the same `tree-l2` / `tree-l3` row as local chats (name + relative time). Heading is `远程 {alias}`. Title: custom overlay, else `<user_query>` first sentence, else cwd basename. Never UUID / `<system-reminder>`. Newest 20 + Load more. Click: `ssh_open_session` then `openSession`. Chat pane keeps OverlayScroll + MessageNodeRail (no extra scroller). Later TODO: Cmd/Ctrl multi-select + right-click delete, same as local select mode. Wave 3 still does not resume grok on the host.
