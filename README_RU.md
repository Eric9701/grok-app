...
<p align="center"><em>Настоящий хост <code>grok</code> CLI — сессии, файлы, удалённый IM, автоматизации и компаньон на рабочем столе</em></p>
...
> ## Примечание
>
> **Grok App не является официальным продуктом xAI.** Это настольный **хост** для локального [Grok Build](https://x.ai) CLI (`grok agent stdio`). Приложение не поставляет модель: диалог, инструменты и уровень рассуждения идут через установленный `grok`.
>
...
4. [Чем это не является](#чем-это-не-является)
5. [Скриншоты](#скриншоты)
6. [Установка и первый запуск](#установка-и-первый-запуск)
7. [macOS: «повреждено» / Gatekeeper](#macos-повреждено--gatekeeper)
8. [Linux: пустое/чёрное окно (WebKit)](#linux-пустоечёрное-окно-webkit)
9. [Linux: sandbox / user namespaces (Ubuntu 24.04+)](#linux-sandbox--user-namespaces-ubuntu-2404)
10. [Пути конфигурации](#пути-конфигурации)
11. [Разработка и сборка](#разработка-и-сборка)
12. [Документация и участие в разработке](#документация-и-участие-в-разработке)
13. [Участники](#участники)
14. [Автор проекта](#автор-проекта)

---

## Обзор

CLI `grok` уже силён в терминале. Для повседневной работы всё ещё нужна рабочая среда: несколько проектов, честная лента инструментов, предпросмотр файлов, расписание, удалённые каналы и интерфейс на языке системы.

**Grok App** — эта рабочая среда. Она **не заменяет** CLI.

1. Установите приложение и подготовьте Grok Build CLI
2. Добавьте проект / создайте новую сессию
3. Подключите агента и общайтесь в режиме **Ask** (по умолчанию) или **YOLO**
4. Просматривайте артефакты, создавайте автоматизации, пишите из IM, управляйте аккаунтом и relay в Settings

**Модель по умолчанию:** Grok **4.6** с Extra High (`xhigh`). Grok 4.5 остаётся доступным.  
...
- **Настоящие Build-сессии** — ACP `grok agent stdio`, FSM сессии у хоста; Ask / once / session / Deny; YOLO на уровне проекта; Grok 4.6 Extra High по умолчанию
- **Рабочая среда для нескольких проектов** — пространства проектов, Kanban агентов, git worktree, fork из ответа ассистента, вложение другого чата, терминал под чатом
- **Файлы в цикле** — предпросмотр и правки изображений / видео / PDF / Office / кода, Changes, Imagine из composer, рекомендуемый плагин ChatCut
- **Вдали от стола** — Remote IM (Feishu/Lark, Telegram, Discord, Slack, DingTalk, WeCom, WeChat, QQ, Matrix, LINE, Weibo), зеркало телефона, локальный session API для других приложений на этой машине
- **Компаньон** — overlay-питомец (Look / Bubbles), Live Voice, диктовка с кастомным STT, скины и обои
- **Ключи и CLI остаются у вас** — квота SuperGrok + heatmap, кастомные relay (OpenRouter, DeepSeek, AI98PRO, Grok Build-compatible), системное хранилище ключей, **15 языков интерфейса**
...
|------|----------------|
| **Проекты и сессии** | Доверенные каталоги, пространства проектов, виртуализированная боковая панель, архив / orphan, перенос чатов между проектами, импорт / открытие CLI-сессий |
| **Параллельные чаты** | Активные ответы продолжают поступать после переключения; лимиты процессов и освобождение простаивающих; Kanban: Needs you / Working / Done |
| **Git worktrees** | Связанные worktree в чипе проекта; переключение cwd одним нажатием (скрыто для не-git) |
| **Fork и rewind** | Fork из завершённого ответа ассистента; rewind / bootstrap дочернего агента |
| **Вложение чата** | `/attach-chat`, `+` в composer или перетаскивание чата из боковой панели (макс. 3; исходный чат не меняется) |

### Чат и агент

| Область | Что доступно |
|------|----------------|
| **Лента** | Think → tools → ответ в порядке потока; живая полоса «working»; нет раннего Ready, пока открыты инструменты или разрешения |
| **Composer** | Очередь последующих сообщений во время работы агента; **Ctrl+Enter** направляет текущий ход; фокус по набору; история промптов; чипы цитат / комментариев |
| **Разрешения** | Ask по умолчанию; разрешить один раз / на сессию / запретить; YOLO; уровень **для каждого проекта**; sandbox рабочей области (Linux — ниже) |
| **Plan / Goal** | Закреплённый прогресс; Markdown-план и шаги в панели ресурсов; Goal — конечная цель, не таймер |
| **Среда выполнения** | Отмена при зависании; структурированные ошибки; диагностический ZIP; Doctor; прерванный ход остаётся прерванным после сбоя |

### Файлы, медиа, Imagine

| Область | Что доступно |
|------|----------------|
| **Resources** | Предпросмотр и **редактирование с сохранением** текста / кода (CodeMirror); несколько вкладок; обновление с диска после записи агентом |
| **Changes** | Diff сессии + git рабочей области; accept / reject / restore с честными пустыми и ошибочными состояниями |
| **Медиа** | Изображения, видео, PDF, Office; относительные markdown-картинки находятся после `curl -o`; экспорт share-card PNG |
| **Создание** | **Create Image / Create Video** в composer, если доступен skill Imagine; Design Mode бокового браузера для localhost-превью |

### Расширения и автоматизации

| Область | Что доступно |
|------|----------------|
| **Slash · Skills** | Палитра slash (как в Grok Build), чипы навыков, `/workflow` / `/workflows` |
| **MCP / Plugins** | Settings → Extensions: MCP, каталог плагинов (openai/plugins + свои источники), Skills, Agents, Hooks — без автоустановки |
| **ChatCut** | Рекомендуемый Codex-плагин; обновление OAuth на хосте; редактор открывается в системном браузере |
| **Автоматизации** | Список запланированных задач; создание естественным языком из чата (скрытая служебная разметка, без JSON в UI) |

### Удалённый доступ, голос, компаньон

| Область | Что доступно |
|------|----------------|
| **Remote IM** | Settings → Remote control: GUI-привязка Feishu/Lark, Telegram, Discord, Slack, DingTalk, WeCom, WeChat, QQ, Matrix, LINE, Weibo (`/p` проект, `/r` возобновить) |
| **Зеркало телефона** | Локальное SPA с токеном; опциональный Cloudflare Quick Tunnel (`cloudflared` или образ Docker) |
| **Session API** | Loopback: список и продолжение по id (`GET /v1/sessions`, `POST /v1/sessions/{id}/turns`); опциональная команда `grok-app` |
| **Голос** | Live Voice; диктовка в composer; кастомный OpenAI-совместимый STT с ключами в системном хранилище |
| **Питомец** | Overlay bloub поверх окон; вкладки Look / Bubbles; празднует, когда последний живой ход готов |

### Аккаунт, внешний вид, языки

| Область | Что доступно |
|------|----------------|
| **Аккаунт и квота** | Переключение нескольких аккаунтов, официальный вход, полоса SuperGrok + heatmap (`/usage`, `/cost`), локальная статистика custom provider |
| **Пользовательские relay** | Независимый `GROK_HOME`, когда App должен писать `config.toml`; режим **shared** (по умолчанию) не трогает `~/.grok` |
| **Пресеты** | Один клик: OpenRouter, DeepSeek, AI98PRO, Amux / Yun, плюс generic OpenAI-compatible и Grok Build-compatible |
| **Внешний вид** | Светлая / тёмная / как в системе, скины, обои (файл / X / Imagine), шрифты UI и терминала, логотип share-card |
| **i18n** | 15 локалей (de, en, es, fil, fr, id, it, ja, ko, pt-BR, ru, ta, uk, zh, zh-TW). Первый запуск следует языку ОС |
| **Пакеты** | macOS ARM / Intel · Windows x64 (установщик + portable) · Linux x64 (AppImage / deb / rpm) |
| **Безопасность** | Системное хранилище ключей (`secrets.json` 0600 как fallback); блокировки записи хранилища; только подтверждения внутри приложения — без `window.confirm` |

---

## Чем это не является

- **Не официальный продукт xAI.** Квота SuperGrok не прилагается. Без рабочего Grok Build CLI / входа это не полноценный агент.
- **Не хост модели.** Официальный Grok, кастомные relay и инструменты идут через локальный CLI.
- **Аккаунт необязателен.** Достаточно уже авторизованного CLI. Вход в приложении нужен для квоты, нескольких аккаунтов и официальных extras.
- **Разрешения по умолчанию — Ask.** YOLO / полный доступ — явный выбор.
- **Режим shared не переписывает `~/.grok` `config.toml`.** Используйте **independent**, если App должен писать провайдеров, privacy или workflows.

---

## Скриншоты

> Из недавней development-сборки для macOS. Макет соответствует текущей рабочей среде.

| Рабочая среда | Аккаунт и квота |
|:---:|:---:|
...
В режиме **shared** (продуктовый default) сессии могут использовать `~/.grok`; режим **independent** использует `agent-home/`.
