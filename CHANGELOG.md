...
- **Sidebar session tree lives in `WorkbenchSessionTree`**: project/orphan paint and the multi-select bar. AppWorkbench keeps open/new-chat and UserMenu.
...
- **侧栏会话树收进 `WorkbenchSessionTree`**：项目/无归属会话绘制和多选条。AppWorkbench 只留打开/新建会话和 UserMenu。
